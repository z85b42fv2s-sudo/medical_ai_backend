...
const backendUrl = window.__BACKEND_URL__ || "http://127.0.0.1:8001";
const FSE_URL =
...
const el = {
  ...
  signupPasswordBanner: document.getElementById("signupPasswordBanner"),
  signupPasswordValue: document.getElementById("signupPasswordValue"),
...
const buttons = {
  ...
  downloadAllDocs: document.getElementById("downloadAllDocsBtn"),
...
function openSignupModal(prefill = {}) {
  ...
  if (el.signupPasswordBanner) {
    el.signupPasswordBanner.classList.add("hidden");
  }
  if (el.signupPasswordValue) {
    el.signupPasswordValue.textContent = "";
  }
}
...
function switchTab(tabId) {
  ...
  if (["profileTab", "indexedTab", "specialtyTab"].includes(tabId) && state.patientId && state.token) {
    loadDocuments();
  }
}
...
async function downloadAllDocumentsHandler() {
  if (!state.patientId || !state.token) {
    setStatus("Effettua l'accesso per scaricare i tuoi documenti.", "error");
    return;
  }
  const encodedPatient = encodeURIComponent(state.patientId);
  const url = `${backendUrl}/patients/${encodedPatient}/documents/download-all`;
  setStatus("Preparazione dell'archivio documenti in corso...", "info");
  try {
    ...
    let filename = `${state.patientId.replace(/\s+/g, "_")}_documenti.zip`;
    ...
    setStatus("Download dei documenti avviato con successo.", "success");
  } catch (error) {
    setStatus(`Impossibile scaricare tutti i documenti: ${error.message}`, "error");
  }
}
...
async function signupSubmitHandler() {
  ...
  try {
    ...
    const infoMessage = data?.message || "Registrazione completata.";
    const escapedMessage = escapeHtml(infoMessage);
    if (statusEl) {
      if (password) {
        const escaped = escapeHtml(password);
        statusEl.innerHTML = `${escapedMessage}<br><small>La password temporanea è evidenziata qui sopra. Copiala e poi chiudi questa finestra.</small>`;
        if (el.signupPasswordValue) {
          el.signupPasswordValue.textContent = password;
        }
        if (el.signupPasswordBanner) {
          el.signupPasswordBanner.classList.remove("hidden");
        }
      } else {
        statusEl.textContent = infoMessage;
      }
    ...
    setStatus(infoMessage, data?.email_sent ? "success" : "error");
  } catch (error) {
    if (statusEl) {
...
function registerEventListeners() {
  ...
  buttons.downloadAllDocs?.addEventListener("click", downloadAllDocumentsHandler);
