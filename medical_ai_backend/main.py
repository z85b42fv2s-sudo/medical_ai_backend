...
from backend.email_service import send_signup_credentials
...
def ask_patient_ai(
    ...
    if mode not in {"cached", "direct"}:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Modalità richiesta non supportata")
    if mode == "direct":
        try:
            analyze_patient(
                patient_id,
...
def public_signup(req: SignupRequest) -> SignupResponse:
    ...
    email_sent = False
    try:
        email_sent = send_signup_credentials(str(req.email), patient_id, password)
    except Exception as exc:
        print(f"[WARN] Invio email registrazione fallito per {patient_id}: {exc}")
    if email_sent:
        print(f"[INFO] Credenziali inviate via email a {req.email}")
    else:
        print(f"[INFO] Email non inviata per {req.email}. Password restituita nella risposta API.")

    return SignupResponse(status="ok", patient_id=patient_id, password=password)
